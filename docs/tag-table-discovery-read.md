...
No separate inventory-tree payload was supplied. Record the user's overall successful workflow report without claiming exhaustive native group, field or ordering verification. No routine replay of the FIO read is needed. The next bounded implementation is get_cross_references, followed by the full eleven-tool MCP cutover.
