> Current continuation point — 2026-09-21: source transition, block inventory/detail and UDT inventory/detail are implemented. The user reported successful block and UDT checks; see [transition status](rehaul-transition.md), [block evidence](get-block.md) and [UDT evidence](udt-discovery-read.md). list_tag_tables and get_tag_table are implemented with [user-supplied FIO evidence](tag-table-discovery-read.md); populated constants remain unverified. Next bounded increment: get_cross_references. The full eleven-tool MCP cutover follows. The snapshot and starting instructions below are the preserved incoming handoff.
