using Backend = FakeConnectionBackend;
...
internal static class ConnectionRegistryTests
{
    ...
    {
        yield return ("connections: independent connections and idempotent connect", IndependentConnections);
        yield return ("connections: stale queued work cannot use a later attachment", QueuedRequest);
        yield return ("connections: path change, close and projectless-to-open invalidate", PathTransitions);
        yield return ("connections: same path with different native identity invalidates", SamePath);
        yield return ("connections: unusable retained proxy invalidates despite equal identity", DeadProxy);
        yield return ("connections: post-read transition discards payload", DuringRead);
        ...
        yield return ("connections: shutdown releases all attachments", Shutdown);
        yield return ("connections: baseline transition during connection fails", DuringConnect);
        yield return ("connections: open project starts one window and refuses an open path", OpenProject);
        yield return ("connections: worker affinity enforced before native access", WorkerAffinity);
