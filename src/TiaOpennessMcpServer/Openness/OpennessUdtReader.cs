...
internal sealed class OpennessUdtReader
{
    private readonly ObjectIdentifierProvider _identifiers;

    private OpennessUdtReader(ObjectIdentifierProvider identifiers) => _identifiers = identifiers;
    ...
    public static BlockInventory Read(Project project, int processId, string plcObjectId, Action validate)
    {
        ...
        var adapter = new OpennessUdtReader(resolved.Identifiers);
        var root = new BlockInventoryNode { Kind = "scope", ScopeType = "plcSoftware", Name = () => plc.Name };
        root.Compositions.Add(() => One(adapter.Group(plc.TypeGroup, false)));
    ...
    private BlockInventoryNode Unit(PlcUnitBase unit, bool safety)
    {
        ...
        node.Compositions.Add(() => One(Group(unit.TypeGroup, safety)));
        return node;
    }
    ...
    private BlockInventoryNode Group(PlcTypeGroup group, bool safety)
    {
        ...
        if (group is PlcTypeSystemGroup root)
            node.Compositions.Add(() => root.SystemTypeGroups.Select(child => SystemGroup(child, safety)));
        return node;
    }

    private BlockInventoryNode SystemGroup(PlcSystemTypeGroup group, bool safety)
    {
        var node = new BlockInventoryNode { Kind = "typeGroup", Name = () => group.Name, IsSystem = true, InSafetyUnit = safety };
        node.Compositions.Add(() => group.Types.Select(type => Udt(type, true, safety)));
        return node;
    }

    private BlockInventoryNode Udt(PlcType type, bool system, bool safety) => new()
    {
        Kind = "udt", Name = () => type.Name,
        ObjectId = () => DiscoveryValues.Nonblank(_identifiers.GetIdentifier(type)),
