...
- Keep definitions, schemas, dispatch, tests and documentation aligned with the eleven-read/eight-write publication in `docs/project-rehaul.md` and `docs/write-operations.md`. The MCP contract harness compiles the production boundary from Program.cs with only the executable bootstrap excluded; no second implementation or listener is used.
