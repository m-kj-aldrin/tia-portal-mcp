> Historical record only. The opt-in startup, normal-V1 mode and read-only publication below are retired. Use [current documentation](../../docs/README.md) for active guidance. The original evidence remains scoped to its recorded build and scenario.

# Connection prototype

This is the connection prototype and its subsequent discovery increment from [project-rehaul.md](../../docs/project-rehaul.md). It uses the existing net48/x64 executable, loopback HTTP listener and shared STA worker. It is a development experiment, not the new MCP tool contract.
...
Still pending live verification: replacing a project with a different project, path changes, a projectless attachment gaining a project, transitions while the dashboard is hidden, a transition during a read, queued work across reconnection, process exit/PID reuse, and guard overhead. Headless lifecycle is omitted from the baseline, not left as a pending check. Existing offline coverage remains separate evidence for the scenarios it simulates.
